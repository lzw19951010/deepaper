...
##### 痛点 (The Gap)
Because 现有开源模型（如 Llama 3.1、OLMo 2）只公开最终权重而隐藏训练数据和中间检查点 → Therefore 社区无法复现训练过程、无法进行可控消融实验 → Therefore 开源 LLM 的科学可重复性严重受损。如 Figure 2 所示，OLMo 3 的模型流程涵盖从预训练到后训练的完整生命周期。
...
- **无 KL 惩罚:** 替代方案是标准 PPO 的 KL 散度约束。OLMo 发现去掉 KL 后模型更充分利用 RL 信号，MATH 提升 2.1pp。Trade-off：训练不稳定风险增大，靠截断比率控制。
- **非对称裁剪:** 替代方案是对称裁剪（如标准 PPO clip=0.2）。非对称设计让正奖励步幅更大（0.28 vs 0.20），加速学习正确行为。
