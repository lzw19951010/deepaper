...
每个 KEY_FINDINGS 条目、DESIGN_DECISIONS 条目、FORMULAS 条目末尾必须附带页码引用，格式为 `(p.X)` 或 `(pp.X-Y)`。页码从原文中的 `[PAGE X]` 标记获取。

确保覆盖论文的所有 10 页段（pp.1-10, 11-20, ..., 直到最后一页），每个段至少有一个引用。示例：
- "MATH 96.2%, 比 Qwen 3 32B 高 0.8pp (Table 5, p.7)"
- "Decision: 三阶段训练. Alternative: 两阶段. Reason: 长上下文能力需要独立阶段 (pp.22-29)"
