...
def write_paper_note(
    ...
    # v2 frontmatter: prefer writer-supplied fields over arxiv metadata,
    # preserve all v2 structured fields (tags, mechanisms, key_tradeoffs,
    # key_numbers), drop legacy v1 fields (venue, doi, keywords, datasets,
    # metrics, core_contribution, abstract, authors, arxiv_categories).
    frontmatter: dict = {
        # --- Identity (writer takes precedence, arxiv metadata is fallback) ---
        "title": analysis_fm.get("title") or metadata["title"],
        "arxiv_id": analysis_fm.get("arxiv_id") or metadata["arxiv_id"],
        "date": analysis_fm.get("date") or metadata["date"],
        "url": analysis_fm.get("url") or metadata["url"],
        "category": category,
        # --- Index layer (v2 structured fields from writer-overview) ---
        "tldr": analysis_fm.get("tldr", ""),
        "baselines": analysis_fm.get("baselines", []),
        "tags": analysis_fm.get("tags") or tags or [],
        "mechanisms": analysis_fm.get("mechanisms", []),
        "key_tradeoffs": analysis_fm.get("key_tradeoffs", []),
        "key_numbers": analysis_fm.get("key_numbers", []),
